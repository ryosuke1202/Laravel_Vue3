...
<template>  
                ...
                <main class="modal__content" id="modal-1-content">
                    <div v-if="customers.value" class="lg:w-2/3 w-full mx-auto overflow-auto">
                        <table class="table-auto w-full text-left whitespace-no-wrap">
                            ...
                            <tbody v-for="customer in customers.value.data" :key="customer.id">
                                <tr>
                                    <td class="px-4 py-3">
                                        <button @click="setCustomer(customer)" type="button" class="text-blue-400">{{ customer.id }}</button>
                                    </td>
    ...
    <button type="button" data-micromodal-trigger="modal-1" @click="searchCustomers" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 text-lg">検索する</button>
</template>