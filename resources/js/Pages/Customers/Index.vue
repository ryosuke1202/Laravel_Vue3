                                    ...
                                    <div class="lg:w-2/3 w-full mx-auto overflow-auto">
                                        <table class="table-auto w-full text-left whitespace-no-wrap">
                                            ...
                                            <tbody v-for="customer in customers.data" :key="customer.id">
                                                <tr>
                                                    <td class="px-4 py-3"><Link :href="route('customers.show', { customer: customer.id })" class="text-blue-400">{{ customer.id }}</Link></td>
                                                    <td class="px-4 py-3">{{ customer.name }}</td>
                                                    <td class="px-4 py-3">{{ customer.kana }}</td>
                                                    <td class="px-4 py-3 text-lg text-gray-900">{{ customer.tel }}</td>
                                                    <!-- <td class="px-4 py-3">
                                                        <Link as="button" :href="route('customer.edit', { customer: customer.id })" class="flex ml-auto text-white bg-gray-500 border-0 py-2 px-6 focus:outline-none hover:bg-gray-600 rounded">編集</Link>
                                                    </td>
                                                    <td class="px-4 py-3">
                                                        <button @click="deleteItem(customer.id)" class="flex ml-auto text-white bg-red-500 border-0 py-2 px-6 focus:outline-none hover:bg-red-600 rounded">削除</button>
                                                    </td> -->
                                                </tr>
                                            </tbody>
                                        </table>
